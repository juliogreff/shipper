...
func TestApplicationExposesStrategyFailure(t *testing.T) {
	...
	noStep2Message := fmt.Sprintf("failed to execute strategy: \"no step 2 in strategy for Release \\\"%s/%s\\\"\"", contender.release.Namespace, contender.release.Name)
	...
	f.expectedEvents = append(f.expectedEvents,
		fmt.Sprintf("Normal ReleaseConditionChanged [Scheduled True] -> [Scheduled False StrategyExecutionFailed %s]", noStep2Message))
