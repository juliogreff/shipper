...
e2e: install build/e2e.test.$(GOOS)-amd64
	./build/e2e.test.$(GOOS)-amd64 --e2e --kubeconfig ~/.kube/config \
...
build/e2e.test.%-amd64: $(PKG) test/e2e/*
	GOOS=$* GOARCH=amd64 go test -c ./test/e2e/ -o build/e2e.test.$*-amd64
...
build/%.deployment.yaml: kubernetes/%.deployment.yaml build $(if $(USE_IMAGE_NAME_WITH_SHA256),build/%.image.$(IMAGE_TAG),)
